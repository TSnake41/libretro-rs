...
impl RetroEnvironment {
    ...
    pub unsafe fn get<T>(&self, key: u32) -> Option<*const T> {
        let mut val: *const T = core::ptr::null();
...
impl<'a> From<Option<&retro_game_info>> for RetroGame<'a> {
    fn from(game: Option<&retro_game_info>) -> RetroGame<'a> {
        let Some(game) = game else { return RetroGame::None { meta: None } };
